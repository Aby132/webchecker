...
import ApiForm, { apiToForm, buildApiPayload, emptyApiForm } from '../components/ApiForm'
...
export default function Configuration({ initialTab = 'projects' }) {
  ...
  const [formApi, setFormApi] = useState(emptyApiForm())
  ...
  const openCreateApi = () => {
    setFormApi(emptyApiForm(projects[0]?.id || ''))
    setApiModal('create')
  }

  const openEditApi = (api) => {
    setFormApi(apiToForm(api))
    setApiModal(api)
  }
  ...
  const saveApi = async () => {
    setSaving(true)
    try {
      const payload = buildApiPayload(formApi)
      if (apiModal === 'create') {
        await createApi(payload)
        toast.push('API added')
      } else {
        await updateApi(apiModal.id, payload)
        toast.push('API updated')
      }
      setApiModal(null)
      await load()
    } catch (err) {
  ...
  const runConfirm = async () => {
    ...
    try {
      ...
      } else if (confirm.type === 'api') {
        await deleteApi(confirm.id)
        toast.push('API deleted')
      } else {
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1>Configuration</h1>
          <p>Manage projects, websites, APIs, and monitoring settings</p>
        </div>
      </div>
      ...
      {tab === 'email' ? (
        ...
        <div className="panel">
          ...
          {projects.length === 0 ? (
            <div className="empty">
              ...
              <p>Create a project to group your monitored websites and APIs.</p>
        ...
        <div className="panel">
          ...
          {apis.length === 0 ? (
            <div className="empty">
              <h3>No APIs configured yet.</h3>
              <p>Add an API endpoint to monitor independently from websites.</p>
              <button className="btn btn-primary" onClick={openCreateApi} disabled={!projects.length}>
                Add API
              </button>
            </div>
          ) : (
              ...
              <table className="data">
                ...
                <tbody>
                  {apis.map((api) => (
                    <tr key={api.id}>
                      <td>{api.name}</td>
                      <td>{projectName(api.project_id)}</td>
                      <td className="mono">{api.method}</td>
                      <td className="url-cell mono">{api.url}</td>
                      <td className="mono">{api.check_interval}s</td>
                      <td><StatusBadge status={api.enabled ? api.status : 'DISABLED'} /></td>
                      <td style={{ display: 'flex', gap: 6 }}>
                        <button className="btn btn-sm" onClick={() => openEditApi(api)}>Edit</button>
                        <button className="btn btn-sm" onClick={() => toggleApi(api)}>
                          {api.enabled ? 'Disable' : 'Enable'}
                        </button>
                        <button
                          className="btn btn-sm btn-danger"
                          onClick={() => setConfirm({ type: 'api', id: api.id, name: api.name })}
                        >
                          Delete
                        </button>
                      </td>
      ...
      {apiModal && (
        <Modal
          title={apiModal === 'create' ? 'Add API' : 'Edit API'}
          wide
          onClose={() => setApiModal(null)}
          footer={
            <>
              <button className="btn" onClick={() => setApiModal(null)}>Cancel</button>
              <button className="btn btn-primary" disabled={saving} onClick={saveApi}>
                {saving ? 'Saving…' : 'Save'}
              </button>
            </>
          }
        >
          <ApiForm form={formApi} setForm={setFormApi} projects={projects} />
        </Modal>
      )}
        ...
        >
          <p>
            ...
            {confirm.type === 'project'
              ? ' This will also remove associated websites, APIs, and their log entries.'
